...
#calculate fact coord by robot coord and recognition coord(eyes in hand)
def cal_fact(cur_bc, recog_co):
	co = cur_bc.copy()
	co[0] -= recog_co[0]
	co[1] += recog_co[1]
	co[2] -= recog_co[2]
...
if __name__ == "__main__":
	...
	camera = UVCCamera(2, mtx, dist)  #randomly transforms between 0 and 4
	...
	ml.send_angles([0,0,0,-90,0,90,0], sp)
	#only rotate rz
	print("r", recog_co)
	#offset = (-recog_co[5] + cur_bcl[5]) * 180 / np.pi
	offset = -recog_co[5] * 180 / np.pi
	if offset > 90:
		offset -= 180
	if offset < -90:
		offset +=180
